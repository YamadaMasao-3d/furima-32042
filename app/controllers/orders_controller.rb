class OrdersController < ApplicationController
  ...
  def buyer_info_params
    item = Item.find(params[:item_id])
    params.require(:buyer_info).permit(:postal_code, :prefecture_id, :municipality, :address, :building_name, :phone_number).merge(user: current_user, item: item, token: params[:token])
  end
